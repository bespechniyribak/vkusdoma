...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def get_cart_products(self, obj):
        return '\n'.join([str(cart.product.title) + ', ' + str(cart.quantity) + 'шт' for cart in obj.carts.all()])

    class Meta:
        model = Order
        fields = ['id',
                  'user_id', 'name', 'total_price', 'adress', 'is_paid', 'is_finished', 'created_at', 'updated_at',
                  'username', 'cart_count', 'cart_products']
