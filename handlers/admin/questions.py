    ...
    if len(questions) == 0:
        ...
        for question in questions:
            ...
            markup.add(InlineKeyboardButton(
                'Ответить',
                callback_data=question_cb.new(cid=question['id'], action='answer')))

            await message.answer(
                f"Chat_id : {question['user_id']}\nUser: {question['full_name']}\nUsername: @{question['username']}\nВопрос: {question['question']}",
                reply_markup=markup)
...
async def process_send_answer(message: Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await bot.send_message(chat_id=question['user_id'], text=text, reply_to_message_id=question['message_id'])
